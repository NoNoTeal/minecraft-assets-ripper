...
for(var folder in files) {
    ...
    if(!filenames[file].endsWith(`.jar`)) continue;
    if(!filenames[file].startsWith(`${versionname}`)) continue;
    dirToVer.push(`${versions}${files[folder]}/${filenames[file]}`)
    nameOfVer.push(`${filenames[file]}`)
    }
}
...
console.log(`Copied file over. Executing Java code... `)
