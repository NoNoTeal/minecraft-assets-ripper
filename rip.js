"use strict";
console.log(`Remember if you modify your index file, thats on you and this app may not work as intended.`)
const fs = require('fs')
...
Directory: ${getFileName.split('/').slice(0, -1).join('/')} | Filename: ${getFileName.split('/').slice(-1).join('/')}`)
    if(!fs.existsSync(`./${version}/assets/${getFileName.split('/').slice(0, -1).join('/')}`)) {
    console.log(`Making directory ./${version}/assets/${getFileName.split('/').slice(0, -1).join('/')}`)
    fs.mkdirSync(`./${version}/assets/${getFileName.split('/').slice(0, -1).join('/')}`, {recursive: true})
    }
    console.log(`Writing ${objects}${folder}/${filenames[file]} as ./${version}/assets/${getFileName}`)
    fs.createReadStream(`${objects}${folder}/${filenames[file]}`).pipe(fs.createWriteStream(`./${version}/assets/${getFileName}`))
    }
